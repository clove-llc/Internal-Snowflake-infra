...
個人で完結して試したい場合は https://signup.snowflake.com/ のトライアル(30日無料、カード不要)でも全章そのまま実施できる。その場合ロールは `ACCOUNTADMIN` を使い、リソース名の読み替え(tutorial/README)は不要。
...
export SNOWFLAKE_ROLE="<演習用ロール>"           # トライアルの場合は ACCOUNTADMIN
```
