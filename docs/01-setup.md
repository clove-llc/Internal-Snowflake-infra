...
実環境を壊す心配のないトライアルアカウントを使う。

1. https://signup.snowflake.com/ から登録(30日間無料、カード不要)。エディションは Standard、クラウドは AWS・東京リージョン
2. ログイン後、**アカウント識別子**を控える。Snowsight 左下のアカウントメニュー → Account → 「アカウント識別子をコピー」で `MYORG-XY12345` の形式が取れる。前半が organization name、後半が account name
...
- `ACCOUNTADMIN` を使うのはチュートリアルで DB・ロールの作成まで行うため。実運用では専用サービスユーザー + 絞ったロールに切り替える(07章)
