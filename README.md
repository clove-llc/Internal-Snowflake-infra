...
├── tutorial/    # 演習の作業場所。tutorial/<名前>/ で作業し PR を出す
...
前提: 共有サンドボックスアカウントの接続情報(管理者から受け取る。docs/01)、ターミナルと Git の基本操作。Terraform の知識は不要。

## 演習時の注意

- 演習は共有サンドボックスアカウントで行う。作成するリソースは `TUTORIAL_<名前>_` プレフィックスで統一し、実環境および他の受講者と区別する([tutorial/README](tutorial/README.md))
